...
TOKEN = 'Bearer eyJhbGciOiJIUzUxMiJ9.eyJpc3MiOiJQaXBlZnkiLCJpYXQiOjE3MzE1MzAyMDYsImp0aSI6IjY3MzQ2YmUzLWEyOWEtNDhiZS1iZmQxLWNiNGNhMjIyMDZkNyIsInN1YiI6MzA1NDUxODE4LCJ1c2VyIjp7ImlkIjozMDU0NTE4MTgsImVtYWlsIjoiYWxlY2VzYXIyN0B5YWhvby5jb20uYnIifX0.N_ZXXsyz2_wBnE8iPC2NDrVSzRkrVtmbQLshNvhssVzPb53QjWN5u6cRc4Ow55aZUrrWyEYvWQ7WexrFu0Jmjg'
...
def obter_nome_organizacao(org_id ):
    query = """
    ...
    if response.status_code == 200:
        ...
        if 'errors' in result:
            # Caso haja erro, como permissão negada
            print("Erro:", result['errors'])
            return None
        else:
            # Retorna o nome da organização
            return result['data']['organization']['name']
    else:
...
def create_pipe(name, organization_id):
    # Definindo a consulta GraphQL 
    mutation = """
    mutation {
      createPipe(input: {name: "%s", organization_id: %d}) {
        clientMutationId
      }
    }
...
def list_organizations():
    ...
    """ # Substitua "ID_DA_ORGANIZACAO" pelo ID da sua organização
...
def execute_graphql_query(org_id):
...
#nome_organizacao = obter_nome_organizacao(301424863)
#print("Nome da Organização:", nome_organizacao)
#print(obter_nome_organizacao(301424863))
...
#print(execute_graphql_query(301424863))
